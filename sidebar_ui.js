...
(function () {
    ...
    function updateUIForPageType() {
        ...
        const currentFormat = document.getElementById('sp-dl-format')?.value;
        const currentDelivery = document.getElementById('sp-dl-delivery')?.value;
        const currentFolder = document.getElementById('sp-dl-folder')?.value;
        const currentMode = document.getElementById('sp-dl-mode')?.value;

        if (dlContainer) dlContainer.innerHTML = renderDownloadTab();
        if (exContainer) exContainer.innerHTML = renderMetadataTab();

        // Restore Selection State
        if (currentFormat) { const el = document.getElementById('sp-dl-format'); if (el) el.value = currentFormat; }
        if (currentDelivery) { const el = document.getElementById('sp-dl-delivery'); if (el) el.value = currentDelivery; }
        if (currentFolder) { const el = document.getElementById('sp-dl-folder'); if (el) el.value = currentFolder; }
        if (currentMode) {
            const el = document.getElementById('sp-dl-mode');
            // Check if the option still exists (e.g. going from library -> session might remove 'selected')
            if (el) {
                if (el.querySelector(`option[value="${currentMode}"]`)) {
                    el.value = currentMode;
                    // Trigger change event to update warning if needed
                    if (currentMode === 'all') {
                        const warn = document.getElementById('sp-dl-warning');
                        if (warn) { warn.style.display = 'block'; warn.innerText = "⚠️ Warning: Downloads ALL visible items. Use with caution."; }
                    }
                } else {
                    // Option no longer valid for this page type!
                    // Reset to default (usually first option)
                    el.selectedIndex = 0;
                }
            }
        }
    ...
    async function handleDownload(e) {
        ...
        try {
            ...
            // --- ORGANIZATION LOGIC ---
            const folderOpt = document.getElementById('sp-dl-folder')?.value || 'default';
            const timestamp = new Date().toISOString().replace(/[:.]/g, '-');

            let rootName = `${STATE.pageTitle}_Audio_${timestamp}`; // Default

            if (folderOpt === 'smart') {
                // Smart: Just the title, no timestamp
                // Relaxed sanitization: Allow #, —, etc. Just block system reserved chars.
                const safeTitle = (STATE.pageTitle || 'Export').replace(/[\\/:*?"<>|]/g, '').trim();
                rootName = safeTitle || `ProducerAI_Audio_Session`;
            } else if (folderOpt === 'generic') {
                rootName = `ProducerAI_Audio_Export_${timestamp}`;
            }
            ...
            for (const item of songs) {
                ...
                try {
                    ...
                    if (delivery === 'zip') {
                        // ZIP Logic: Add to zip folder
                        // Note: folder is already 'rootName', so file will be inside that folder in the zip
                        folder.file(`${safeName}.${format === 'original' ? 'wav' : format}`, blob);
                    } else {
                        // INDIVIDUAL Logic: Send to Background for Folder Support
                        ...
                        await new Promise(resolve => {
                            reader.onloadend = () => {
                                ...
                                chrome.runtime.sendMessage({
                                    type: "DOWNLOAD",
                                    url: dataUrl,
                                    filename: `${rootName}/${safeName}.${format === 'original' ? 'wav' : format}`,
                                    saveAs: false
                                });
                                resolve();
                            };
                            ...
                            await new Promise(resolve => {
                                reader.onloadend = () => {
                                    ...
                                    chrome.runtime.sendMessage({
                                        type: "DOWNLOAD",
                                        url: dataUrl,
                                        filename: `${rootName}/${safeName}.mp3`,
                                        saveAs: false
                                    });
                                    resolve();
                                };
    ...
    async function handleExport(e) {
        ...
        try {
            ...
            let rootName = `${STATE.pageTitle || 'Export'}_${timestamp}`; // Default

            if (folderOpt === 'smart') {
                // Smart: Just the title, no timestamp (User Request)
                // Relaxed sanitization: Allow #, —, etc. Just block system reserved chars.
                const safeTitle = (STATE.pageTitle || 'Export').replace(/[\\/:*?"<>|]/g, '').trim();
                rootName = safeTitle || `ProducerAI_Session`;
            } else if (folderOpt === 'generic') {
                rootName = `ProducerAI_Export_${timestamp}`;
            }
            ...
            if (pageType === 'session') {
                ...
                // 1. CHAT LOGS (New Selectors)
                // User: div.chat-history-part.user-part
                // Agent: div.chat-history-part.producer-part

                const chatParts = Array.from(document.querySelectorAll('.chat-history-part'));
                let log = "SESSION CHAT LOG\n=================\n\n";

                // Init global dump
                finalRawDump += "SESSION RAW DUMP (Full Export)\n==============================\n\n";
                finalRawDump += "--- CHAT LOGS ---\n\n";

                if (chatParts.length > 0) {
                    chatParts.forEach(part => {
                        const isUser = part.classList.contains('user-part');
                        const isAgent = part.classList.contains('producer-part');
                        ...
                        if (isUser || isAgent) {
                            ...
                            log += `[${sender}]\n${text}\n-----------------\n`;
                            finalRawDump += `[${sender}] ${text}\n\n`;

                            // Check for lyrics in agent messages
                            if (isAgent) {
                    ...
                    // Fallback to body text if no chat parts found
                    const allText = document.body.innerText;
                    finalRawDump += allText + "\n\n";
                    log = "Could not structurally parse chat (New Selectors Failed). See raw dump.";
                }
            ...
            // --- SONG SCRAPING (New Selectors) ---
            setBtn("Scraping Songs...", true);
            let songsToExport = [];

            // Append Header to Dump
            finalRawDump += "\n--- SONGS & PROMPTS ---\n\n";
            ...
            if (isSongLink) {
                ...
                // SESSION / PLAYLIST: Look for Tracks
                // User requirement: "all tracks from session will be inside data-panel-collapsible"

                const collapsiblePanels = Array.from(document.querySelectorAll('[data-panel-collapsible]'));

                // If panels found, search within them.
                if (collapsiblePanels.length > 0) {
                    // console.log(`Found ${collapsiblePanels.length} collapsible panels.`);

                    collapsiblePanels.forEach(panel => {
                        const link = panel.querySelector('a[href^="/song/"]');
                        if (link) {
                            const href = link.getAttribute('href').replace(/\/+$/, '');
                            const uuid = href.split('/').pop();

                            // Check for "Sound" div as prompt source
                            songsToExport.push({ uuid: uuid, element: panel, hasSoundDiv: true });
                        }
                    });
                }

                // Fallback / Cumulative: Check legacy rows too
                    ...
                    songRows.forEach(row => {
                        try {
                            ...
                            if (link) {
                                const href = link.getAttribute('href').replace(/\/+$/, '');
                                const uuid = href.split('/').pop();
                                songsToExport.push({ uuid: uuid, element: row });
                            }
                        } catch (e) { }
                    });
                }
            ...
            if (uniqueSongs.length > 0) {
                ...
                for (const item of uniqueSongs) {
                    ...
                    try {
                        // DOM Fallback Info
                        let domSoundPrompt = "";
                        if (item.hasSoundDiv && item.element) {
                            const divs = Array.from(item.element.querySelectorAll('div'));
                            const soundLabel = divs.find(d => d.innerText.trim() === "Sound");
                            if (soundLabel) {
                                if (soundLabel.nextElementSibling) domSoundPrompt = soundLabel.nextElementSibling.innerText.trim();
                                else if (soundLabel.parentElement && soundLabel.parentElement.nextElementSibling) {
                                    domSoundPrompt = soundLabel.parentElement.nextElementSibling.innerText.trim();
                                }
                            }
                        }
                        ...
                        // Append to Final Raw Dump
                        finalRawDump += `[Track: ${finalTitle}]\nUUID: ${item.uuid}\nModel: ${model}\nSound/Tags: ${sound || domSoundPrompt}\nLyrics/Prompt: ${lyrics}\n-----------------------------------\n\n`;
                ...
                if (metadataRecords.length > 0) {
                    ...
                    // Save Raw Dump
                    dataFolder.file("session_dump.txt", finalRawDump);
                }
            }
    ...
    function extractPageTitle() {
        ...
        // 1. Session Page Specific
        if (window.location.pathname.includes('/session/')) {
            // User-Specified Selector (Highest Priority)
            const userTitle = document.querySelector('div.truncate.text-base.font-semibold');
            if (userTitle && userTitle.innerText.trim()) {
                // console.log("Found User Title Selector:", userTitle.innerText.trim());
                return userTitle.innerText.trim();
            }

            // Look for the main title input
            const titleInput = document.querySelector('input[placeholder="Name your session"]');
            if (titleInput && titleInput.value && titleInput.value.trim()) {
                // console.log("Found Session Input Title:", titleInput.value);
        ...
        if (titleEl && titleEl.innerText.trim()) {
            ...
            if (t !== "Song" && t !== "Producerai Toolsuite") {
                // console.log("Found Editable Title:", t);
                return t;
            }
        }
        ...
        try {
            ...
            if (script) {
                ...
                if (songTitle && songTitle !== "Song" && songTitle !== "Producerai Toolsuite") {
                    // console.log("Found Hydration Title:", songTitle);
                    return songTitle;
                }
            }
        } catch (e) { }
    ...
    function analyzePage() {
        ...
        // Reset state on analysis
        const oldType = STATE.pageType;
        STATE.pageType = 'unknown';

        if (path.includes('/library/my-songs') || path.includes('/library/playlists')) {
    ...
    setInterval(() => {
        ...
        const pathChanged = currentPath !== STATE.lastPath;

        if (pathChanged || titleBad) {

            // Only update lastPath if it actually changed
            if (pathChanged) STATE.lastPath = currentPath;

            const oldTitle = STATE.pageTitle;
            const oldType = STATE.pageType; // Track type change

            analyzePage();

            // If title changed or path changed, update UI
            if (pathChanged || STATE.pageTitle !== oldTitle || STATE.pageType !== oldType) {
                const sidebar = document.getElementById('sp-sidebar');
                // Force update if sidebar is open
